...
Similar to the player, the enemy AI is made using a state machine. The enemy first starts in the move phase where they wander around the platform before a ledge is detected, making them turn arround so that they don't fall. Also if they are detecting a wall they can turn arround so the enemy is not stuck hugging a wall.
![image](https://github.com/TheDarkestNightRises/game-dev/assets/91905169/7c456cf8-175c-4fee-8b2a-313551efca5c)

At certain intervals they change their transition to idle. This transition makes the enemy feel a bit more alive since they wander about instead of walking tirelessly.
![image](https://github.com/TheDarkestNightRises/game-dev/assets/91905169/7e14d150-7e4f-41a0-bd1b-14b05ee9b451)
If an enemy spots the character then they go into the pursue state, while they move faster towards the player. If somehow the player jumps over the enemy or if the enemy loses track of its target then they go into investigate mode, where they try to turn arround and find the player. The enemies have a certain attack range and if they are close to the player they start 
attacking.
![image](https://github.com/TheDarkestNightRises/game-dev/assets/91905169/17bce913-1b73-4f94-b722-ac7fc9f16641)
![image](https://github.com/TheDarkestNightRises/game-dev/assets/91905169/e8fcac95-df43-42bf-9250-82ee71a216be)
The attack state is invoked by playing the attack animation which contains triggers for when the attack has finished. If the attack has finished and the player is in the radius of the attack(represented by a circle) then they will take damage. If their health bar is depleted, the enemy drops the coins as well as dissabling their rigidbody so that the player cannot attack anymore. For a nice touch, the sprite of the enemy sits on the ground for a few seconds, after which is destroyed. If there are a lot of enemies on the map an object pool could be used , but on each level there are arround 2-3 enemies.
